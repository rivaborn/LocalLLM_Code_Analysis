...
On the ollama backend it runs a **pre-flight GPU load** (POSTs the model to the gateway's `/api/load` so Ollama owns the card before the run; `-SkipLoad` to skip). If **`NOTIFY_URL`** is set in `.env` (an ntfy topic URL or any plain-text webhook), it POSTs a **push notification at each section** — pipeline start, every stage (OK/FAILED), and complete — each prefixed with the target name. For long unattended runs it also POSTs the live **progress line** (`PROGRESS: x/y skip=.. fail=.. eta=..`) as a **heartbeat every `NOTIFY_PROGRESS_INTERVAL` seconds** (default 900 = 15 min) during Pass 1 and Pass 2, and includes a **pre-run ETA estimate** (`files × ETA_MIN_PER_FILE`, default 3.3 min/file) in the per-subsystem start notification + banner. **`run_batch.ps1`** runs the pipeline across several targets back-to-back (`-Targets a, b, c`), printing/POSTing a **whole-run ETA breakdown** at the start, preserving each run's report as `architecture/Run Report - <name>.md`, and continuing past a failed subsystem; collect + status updates stay manual, one subsystem at a time.
...
architecture/
  ...
  .archgen_state/       Pass 1 state: hashes.tsv (progress), counter.json (fail/retries), datablob.tsv (data-blob skips)
...
**VRAM headroom & context sizing (ollama).** The model weights + KV cache must leave headroom on the GPU. Running the card near 100% VRAM makes `llama-server` OOM-crash under any transient allocation (or a spike from another GPU process sharing the card), which surfaces mid-run as intermittent `Unable to connect` / `The connection was closed unexpectedly` / `operation timed out` errors and a climbing fail count. KV-cache size scales with `LLM_NUM_CTX`, so lower it until the loaded footprint (check `ollama ps` and `nvidia-smi` on the box; `ollama stop <model>` first to read a clean baseline) leaves a few GB free — for a 27B Q4 model on a 24 GB card, ~64k context (`LLM_NUM_CTX=65536`) lands ~2 GB under the ceiling whereas 96k leaves almost none. **Gotcha:** the pre-flight gateway load (`run_pipeline.ps1` → `POST :11430/api/load`) loads the model at its *baked* Modelfile `num_ctx`, **not** at `LLM_NUM_CTX` — so if `LLM_NUM_CTX` is smaller than the model's baked context, the pre-flight loads at the large baked size (tight/over the ceiling) and the first per-file request then forces a wasteful unload+reload down to `LLM_NUM_CTX`. To keep them consistent, **bake a context-matched Ollama tag** (`ollama create <model>-64k` from a Modelfile `FROM <model>` + `PARAMETER num_ctx 65536`; reuses the weights blob) and point `LLM_DEFAULT_MODEL` at it, so both the pre-flight load and the per-request calls use the same context natively.

Implementation: `llm_core.ps1` (`Get-LLMBackend` / `Get-LLMEndpoint` / `Get-LLMModel` / `Invoke-LocalLLM`) is dot-sourced by every LLM-calling script. Because `*_worker.ps1` run in `Start-Job` runspaces (where `$PSScriptRoot` is empty), they dot-source `llm_core.ps1` via a `toolkitDir` passed from the parent, and receive the resolved backend/endpoint/model/think flags as `-llm*` parameters. Each call site branches on `$llmBackend`: `claude` keeps the original `& claude` path; anything else calls `Invoke-LocalLLM`. The vLLM path posts to `/v1/chat/completions` (OpenAI schema); the ollama path posts to `/api/chat` when `NumCtx>0` (sending `think` when enabled). When `-Think` is on, `Invoke-LocalLLM` floors the token budget (`LLM_THINK_MIN_TOKENS`, 8000) so reasoning + content both fit. On non-claude backends `archgen.ps1` also forces `Jobs=1` and disables file batching + header bundling (local models choke on the big multi-file / multi-header prompts those create) -- each file is sent on its own with its injected LSP context, and any still-oversized file auto-degrades (drop headers, then truncate). With `DEGRADE_FALLBACK_MODEL` set (e.g. `sonnet`), a file that hits the local degrade path (thinking-exhaustion / empty / context overflow) is escalated **once** to that claude model with the full untruncated payload -- producing a quality doc instead of a truncated one -- and the run then continues on the local server (requires a valid `CLAUDE*_CONFIG_DIR`; empty = disabled).
...
**Data-blob detector -- skip giant generated literal tables.** Some headers are precomputed lookup/data tables (e.g. `GlintShadingLUTsData.h`, `LTC.h` -- thousands of hardcoded hex/numeric literals). They have no architecture to document, drive a thinking model to empty output (it exhausts `num_predict` inside `<thinking>`), and overflow even the `DEGRADE_FALLBACK_MODEL` (too big for sonnet's context), so they otherwise fail after wasted retries + a doomed escalation. Both `archgen.ps1` (Pass 1) and `archpass2.ps1` (Pass 2) pre-scan for these in the queue-build step: a file `>= DATA_BLOB_MIN_LINES` (default 2000) whose non-comment lines are `>= DATA_BLOB_FRACTION` (default 0.6) pure numeric/hex literals is **stubbed** like a trivial file instead of being sent to the LLM. Identifiers/keywords never match the literal regex, so real code is not flagged (`GlintShadingLUTsData.h` 0.999 -> blob; `RHI.cpp` 0.003 -> not). Knobs are backend-agnostic and default-on: `DETECT_DATA_BLOB=1`, `DATA_BLOB_MIN_LINES=2000`, `DATA_BLOB_FRACTION=0.6`. Each skip is counted (shown in the pre-run "Files:" line and an end-of-run summary) and appended to `<state>/datablob.tsv`; `run_pipeline.ps1` clears + aggregates both passes' ledgers (`.archgen_state` + `.pass2_state`) and lists them in a **"Data-blob skips"** section of `Run Report.md`.
