...
### llm_scripts/run_pipeline.ps1 — Full Pipeline Orchestrator
One-command end-to-end runner for the whole pipeline (serena_extract → [archgen_dirs, claude-only] → archgen → archxref → archgraph → arch_overview → archpass2_context → archpass2). Runs each stage in order, and **stops after any stage that has failures**: it completes the stage, then writes the failed files + reasons into the report and exits without running later stages. Generates **`Run Report.md`** (default `architecture/Run Report.md`, override with `-ReportPath`) containing run metadata, a per-stage status/duration/counts table, a failures table (file / type / reason), and an outputs summary. Backend-aware: reads `LLM_BACKEND` from `.env` and skips Step 0b (`archgen_dirs`) on non-claude backends. To make a stage *complete* despite failures, it sets `ARCH_CONTINUE_ON_ERROR` so `archgen`/`archpass2` record terminally-failing files to `<state>/failures.tsv` instead of fatal-aborting. Flags: `-TargetDir`, `-Preset`, `-Jobs`, `-Top` (Pass 2), `-Claude1`, `-SkipSerena`, `-SkipPass2`, `-EnvFile`, `-ReportPath`.
...
Per-file worker dispatched by `archpass2.ps1`. Dot-sources `llm_core.ps1` and calls the resolved LLM backend. Uses `file_doc_system_prompt.txt` as a fixed system prompt across all calls to enable prompt caching (claude backend); the per-file prompt schema is embedded in the user message. Auto-detects targeted context files (`.pass2_context/<path>.ctx.txt`) and uses them instead of global truncated blobs when available. Four-stage fallback: stage 0 (source + pass1 + targeted or global context), stage 1 (drop xref/targeted context), stage 2 (drop arch context, truncate harder), stage 3 (source only). On a local backend it also treats a 400/context error or empty/short/thinking-exhausted output as a degrade (not a fatal failure), and — like Pass 1 — when `DEGRADE_FALLBACK_MODEL` is set it escalates that file once to the claude model with the full payload before truncating, then continues locally. Handles rate limits with shared pause file coordination across parallel workers. Under the `run_pipeline.ps1` orchestrator (env `ARCH_CONTINUE_ON_ERROR`), a terminally-failing file is recorded to `.pass2_state/failures.tsv` and the stage continues instead of fatal-aborting.
...
Key-value configuration for the archgen pipeline. Backend selection: `LLM_BACKEND` (`ollama` default / `vllm` / `claude`), `LLM_HOST`, `LLM_PORT`, `LLM_DEFAULT_MODEL`, `LLM_THINK`, `LLM_NUM_CTX`, `LLM_TEMPERATURE`, `LLM_MAX_TOKENS`, `LLM_TIMEOUT`, `LLM_ENDPOINT`. Claude-CLI settings: `CLAUDE_MODEL`, `CLAUDE1_CONFIG_DIR`/`CLAUDE2_CONFIG_DIR`. Reliability: `DEGRADE_FALLBACK_MODEL` (local backends — escalate a degrading file to this claude model; empty = disabled). Core settings: `JOBS`, `MAX_RETRIES`, `BUNDLE_HEADERS`, `MAX_FILE_LINES`, `PROMPT_FILE`, `PRESET`, `INCLUDE_EXT_REGEX`, `EXCLUDE_DIRS_REGEX`, `CODEBASE_DESC`. Token optimization settings: `SKIP_TRIVIAL`, `MIN_TRIVIAL_LINES`, `TIERED_MODEL`, `HIGH_COMPLEXITY_MODEL`, `BUNDLE_HEADER_DOCS`, `BATCH_TEMPLATED`, `USE_MAX_TOKENS`, `JSON_OUTPUT`, `CLASSIFY_FILES`.
